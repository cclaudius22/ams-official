...
**Deferred (planned):**
- **T9 — `LiveQueueMetrics.tsx` — ✅ DONE (2026-06-28).** Agent 1 cleared it (gate passed, commits 7171b14/95cafb0 already in shared tree). Migrated presentation-only onto the shared layer: preserved his pending-count filter (factored into one `PENDING_STATUSES` constant shared by `statusData` + `metrics.pending`); fixed gate finding #7 with `toVisaKey()` (registry-keyed visa colour/label, regression-tested). **Build-error caught by screenshot** (tsc-green but bundler-fail): client component importing registry *values* from the contract barrel pulled server-only `fs/promises` → fixed by importing from `@/config/visaTypes`. Full suite **157 passed / 24 skipped** · tsc **76**. All 3 chart surfaces now migrated.
- **Data-wiring track:** replace the `Math.random()` hooks with a `@/api-contracts/queue-contract` adapter (`LiveApplication[]` → series via `visaTypeId`/`visaTypeLabel`/`RecommendationOutcome`). Both follow-ups above naturally fold into this. (LiveQueueMetrics already reads real props — `toVisaKey` prefers `visaTypeId` once the provider populates it.)
