      ...
      <Card className="flex-1 flex flex-col shadow-sm">
        <CardHeader className="pb-4 border-b">
            {/* ... Input/Textarea for title/desc with onBlur={updateStepDetails} ... */}
             <Label htmlFor={`step-title-${activeStep.id}`} className="sr-only">Title</Label>
             <Input id={`step-title-${activeStep.id}`} value={localTitle} onChange={handleTitleChange} onBlur={updateStepDetails} className="text-lg font-semibold ..." placeholder="Step Title"/>
             <Label htmlFor={`step-desc-${activeStep.id}`} className="sr-only">Description</Label>
             <Textarea id={`step-desc-${activeStep.id}`} value={localDescription} onChange={handleDescriptionChange} onBlur={updateStepDetails} className="text-sm ..." placeholder="Step description (optional)" rows={1}/>
        </CardHeader>
