...
const FieldSettingsModal = ({ isOpen, onOpenChange, stepId, fieldId }: FieldSettingsModalProps) => {
  ...
  const [fieldData, setFieldData] = useState<Partial<FieldConfig>>({}); // Local state for form
  ...
  useEffect(() => {
    if (targetField) {
      setFieldData({
        ...
        // Load other properties here later (placeholder, fieldName etc.)
      });
    } else {
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[525px]"> {/* Adjust width */}
        ...
        <div className="grid gap-4 py-4">
           ...
           {/* Add Placeholder input later */}
           {/* <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="field-placeholder" className="text-right col-span-1">Placeholder</Label>
              <Input id="field-placeholder" ... />
           </div> */}

           {/* Add Field Name input later (potentially advanced/read-only?) */}
            {/* <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="field-fieldName" className="text-right col-span-1">Field Name</Label>
