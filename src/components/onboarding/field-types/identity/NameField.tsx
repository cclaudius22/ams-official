// src/components/onboarding/field-types/identity/NameField.tsx
import React from 'react';
import { useFormContext, Controller } from 'react-hook-form';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { User } from 'lucide-react';

export interface NameFieldProps {
  field: {
    id: string;
    fieldName: string;
    label: string;
    placeholder?: string;
    helpText?: string;
    isRequired?: boolean;
    validationRules?: Array<{
      type: string;
      value?: any;
      message?: string;
    }>;
    fieldType?: 'firstName' | 'lastName';
  };
  disabled?: boolean;
}

export const NameField: React.FC<NameFieldProps> = ({ 
  field, 
  disabled = false 
}) => {
  const { control } = useFormContext();
  
  // Create validation rules
  const validationRules: Record<string, any> = {};
  
  if (field.isRequired) {
    validationRules.required = `${field.label} is required`;
  }
  
  // Add any custom validation rules
  field.validationRules?.forEach(rule => {
    if (rule.type === 'required') {
      validationRules.required = rule.message || `${field.label} is required`;
    } else if (rule.type === 'minLength') {
      validationRules.minLength = {
        value: rule.value,
        message: rule.message || `${field.label} must be at least ${rule.value} characters`,
      };
    } else if (rule.type === 'maxLength') {
      validationRules.maxLength = {
        value: rule.value,
        message: rule.message || `${field.label} must be at most ${rule.value} characters`,
      };
    } else if (rule.type === 'pattern') {
      validationRules.pattern = {
        value: new RegExp(rule.value),
        message: rule.message || `Please enter a valid ${field.label.toLowerCase()}`,
      };
    }
  });
  
  return (
    <div className="space-y-2">
      <Label 
        htmlFor={field.fieldName}
        className={field.isRequired ? 'after:content-["*"] after:ml-0.5 after:text-red-500' : ''}
      >
        {field.label}
      </Label>
      
      <Controller
        name={field.fieldName}
        control={control}
        rules={validationRules}
        defaultValue=""
        render={({ field: formField, fieldState }) => (
          <div className="relative">
            <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground">
              <User className="h-4 w-4" />
            </div>
            <Input
              id={field.fieldName}
              type="text"
              placeholder={field.placeholder}
              className={`pl-9 ${fieldState.error ? 'border-destructive' : ''}`}
              {...formField}
              disabled={disabled}
            />
          </div>
        )}
      />
      
      {field.helpText && (
        <p className="text-xs text-muted-foreground">{field.helpText}</p>
      )}
      
      {fieldState.error && (
        <p className="text-xs text-destructive">{fieldState.error.message}</p>
      )}
    </div>
  );
};

// Preview component for the builder
export const NameFieldPreview: React.FC<NameFieldProps> = ({ field }) => {
  return (
    <div className="space-y-2">
      <Label 
        htmlFor={`preview-${field.fieldName}`}
        className={field.isRequired ? 'after:content-["*"] after:ml-0.5 after:text-red-500' : ''}
      >
        {field.label}
      </Label>
      
      <div className="relative">
        <div className="absolute left-3 top-1/2 transform -translate-y-1/2 text-muted-foreground">
          <User className="h-4 w-4" />
        </div>
        <Input
          id={`preview-${field.fieldName}`}
          type="text"
          placeholder={field.placeholder}
          className="pl-9"
          disabled
        />
      </div>
      
      {field.helpText && (
        <p className="text-xs text-muted-foreground">{field.helpText}</p>
      )}
    </div>
  );
};

// Default props for first name field
export const firstNameFieldDefaultProps = {
  label: 'First Name',
  placeholder: 'Enter your first name',
  helpText: '',
  isRequired: true,
  fieldType: 'firstName',
  validationRules: [
    {
      type: 'minLength',
      value: 2,
      message: 'First name must be at least 2 characters'
    }
  ]
};

// Default props for last name field
export const lastNameFieldDefaultProps = {
  label: 'Last Name',
  placeholder: 'Enter your last name',
  helpText: '',
  isRequired: true,
  fieldType: 'lastName',
  validationRules: [
    {
      type: 'minLength',
      value: 2,
      message: 'Last name must be at least 2 characters'
    }
  ]
};

// The icon for the component library
export const NameFieldIcon = User;