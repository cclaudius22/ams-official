...
export async function buildOrLoadVectorStore(
  ...
  let collection;
  try {
    collection = await client.getCollection({ name: collectionName, embeddingFunction: () => { throw new Error("Embedding function not implemented"); } });
  } catch {
    collection = await client.createCollection({ name: collectionName, embeddingFunction: () => { throw new Error("Embedding function not implemented"); } });
  }
