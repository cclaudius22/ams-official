...
export default function VerificationHub() {
  ...
  // Run all checks in sequence for selected systems
  const runAllChecks = async () => {
    ...
    const selectedCompatibleSystems = systems.filter(
      s => selectedSystems.includes(s.id) && s.inputType === inputType
    );

    for (const system of selectedCompatibleSystems) {
          ...
          >
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                {/* Checkbox for selecting a system */}
                <input
                  type="checkbox"
                  className="h-5 w-5"
            ...
            <div className="space-y-3">
              ...
              {/* Action Button for individual checks */}
              <Button
                variant="outline"
                className={`w-full ${
                  !isSystemCompatible(system.inputType) ? 'opacity-70' : ''
                }`}
                onClick={() => runSystemCheck(system.id)}
                disabled={
                  !documentId ||
                  !!checkingSystem ||
                  !isSystemCompatible(system.inputType)
                }
                title={
                  !isSystemCompatible(system.inputType)
                    ? `Requires ${system.inputType} input`
                    : ''
                }
              >
