...
// Keep Card imports if used elsewhere, otherwise could remove
    ...
    <Accordion type="single" collapsible className="w-full mb-6 border rounded-lg overflow-hidden shadow-sm" defaultValue='ai-scan-results'>
      <AccordionItem value="ai-scan-results" className="border-b-0">
        ...
        <AccordionContent className="p-4 md:p-6 bg-white">
          ...
          {/* Tabs for Details */}
          {/* ***** CHANGE 1: Set defaultValue to "analysis" ***** */}
          <Tabs defaultValue="analysis" className="mt-6">
            <TabsList className="grid w-full grid-cols-2 gap-2 bg-transparent p-0 h-auto mb-4">
              {/* ***** CHANGE 2: Update Trigger Styles ***** */}
              <TabsTrigger
                value="issues"
                className={cn(
                  // Base styles for BOTH triggers (inactive state)
                  "inline-flex items-center justify-center whitespace-nowrap rounded-md text-xs font-medium ring-offset-background transition-all",
                  "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
                  "disabled:pointer-events-none disabled:opacity-50",
                  // Specific INACTIVE styles
                  "border border-input bg-background hover:bg-accent hover:text-accent-foreground h-9 px-3 py-2", // Mimics outline but ensures bg
                  "text-muted-foreground", // Use a muted text color for inactive
                  // Active state overrides (using primary color)
              ...
              {/* ***** END OF CHANGES for Triggers ***** */}
            </TabsList>

            {/* Issues Tab Content */}
            <TabsContent value="issues" className="pt-0">
            ...
            <TabsContent value="analysis" className="pt-0 text-sm space-y-4">
