...
import {
  ...
  AlertTriangle,
  RefreshCw,
  ChevronDown, // Often used implicitly by AccordionTrigger
  FileText,
  Target,
  Anchor
} from 'lucide-react'
...
const SecurityCheckItem: React.FC<{ label: string; status: 'passed' | 'warning' | 'failed' | 'info' }> = ({ label, status }) => {
  const icons = {
    ...
    info: <CheckCircle2 className="h-4 w-4 text-blue-500" /> // Or a different icon for info
  };
  const colors = {
...
export default function AIScanResultsRedesigned({ scanResult, onRefreshScan }: AIScanResultsProps) {
  ...
  const formatConsistentDateTime = (dateInput: string | Date | undefined | null): string => {
    ...
    try {
      return new Intl.DateTimeFormat('en-CA', { // Use 'en-CA' or 'sv-SE' for YYYY-MM-DD
        year: 'numeric', month: '2-digit', day: '2-digit',
        hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false
      }).format(new Date(dateInput)).replace(',', ''); // Remove comma if present
    } catch (e) {
  ...
  const rootednessScore = scanResult.rootednessScore ?? 84; // Use nullish coalescing for default
  ...
  const securityChecks = [
    ...
    { label: 'Visa History', status: 'warning' as const }, // Example warning
    { label: 'Financial Check', status: 'passed' as const },
    // Add more checks if needed
  ];
  // --- END MOCK ---

  const getIssueSeverityClass = (severity: Issue['severity']) => {
    switch (severity) {
      case 'critical':
      case 'high':
  ...
  return (
    <Accordion type="single" collapsible className="w-full mb-6 border rounded-lg overflow-hidden" defaultValue='ai-scan-results'>
      <AccordionItem value="ai-scan-results" className="border-b-0">
        <AccordionTrigger className="bg-gray-50 hover:bg-gray-100 px-4 py-3 text-base hover:no-underline">
          <div className="flex justify-between items-center w-full">
            <div className="flex items-center">
              <Shield className="h-5 w-5 text-blue-600 mr-3" />
              <span className="font-semibold text-gray-800">AI Assessment Results</span>
            </div>
            <div className="flex items-center space-x-2">
                 <Badge variant="outline" className={`px-2 py-0.5 text-xs font-medium ${risk.badgeColor}`}>
                    Risk: {risk.level}
                 </Badge>
                 <Badge variant="secondary" className="px-2 py-0.5 text-xs font-medium">
                    Score: {scanResult.score}/100
                 </Badge>
                {/* ChevronDown icon is usually added automatically by AccordionTrigger */}
            </div>
          </div>
        </AccordionTrigger>
        ...
        <AccordionContent className="p-4 md:p-6 bg-white">
          ...
          <Tabs defaultValue="issues" className="mt-4">
            ...
            {/* Analysis Tab */}
            <TabsContent value="analysis" className="pt-4 text-sm space-y-4">
               <div className="p-3 border rounded-md bg-gray-50">
                 ...
                 <p className="text-gray-600 text-xs leading-relaxed">
                   {scanResult.rootednessSummary || `The analysis suggests [moderate/strong/weak] ties to the applicant's home country based on factors like employment stability, property ownership, and family connections. The calculated rootedness score is ${rootednessScore}/100.`}
                 </p>
               </div>
               <div className="p-3 border rounded-md bg-gray-50">
                 ...
                 <p className="text-gray-600 text-xs leading-relaxed">
                   {scanResult.intentSummary || `Based on the provided travel plans, financial capacity, and stated purpose of visit, the applicant's intent appears to be [consistent/partially consistent/inconsistent] with a temporary stay. The intent score is ${intentScore}/100. Factors considered include [mention key factors].`}
                 </p>
               </div>
               <div className="p-3 border rounded-md bg-gray-50">
                 ...
                 <p className="text-gray-600 text-xs leading-relaxed">
                   {scanResult.documentSummary || `Document verification checks indicate [general status, e.g., overall consistency, minor discrepancies found]. Key documents like the passport and financial statements have been reviewed for authenticity and consistency. [Add specific flags if any, e.g., 'Passport validation passed, minor inconsistencies noted in bank statement dates'].`}
                 </p>
               </div>
            </TabsContent>
