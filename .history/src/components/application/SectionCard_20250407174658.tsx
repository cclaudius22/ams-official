        ...
        <div>
          ...
          <div className="p-4 bg-white rounded-md border border-gray-200">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-xs text-gray-500">Date of Arrival</p>
                <p className="text-sm font-medium" suppressHydrationWarning>{formatDate(data.dateOfArrival)}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Date of Departure</p>
                <p className="text-sm font-medium" suppressHydrationWarning>{formatDate(data.dateOfDeparture)}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Duration of Stay</p>
                <p className="text-sm font-medium">{data.durationOfStayDays} days</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Intended Entries</p>
                <p className="text-sm font-medium capitalize">{data.intendedEntries}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Arrival City</p>
                <p className="text-sm font-medium">{data.arrivalCity || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Mode of Transport</p>
                <p className="text-sm font-medium">{data.modeOfTransport || 'N/A'}</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div>
            ...
            <div className="p-4 bg-white rounded-md border border-gray-200">
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <p className="text-xs text-gray-500">Type</p>
                  <p className="text-sm font-medium">{data.accommodation.type || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Name/Host</p>
                  <p className="text-sm font-medium">{data.accommodation.name || 'N/A'}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-xs text-gray-500">Address</p>
                  <p className="text-sm font-medium">{data.accommodation.address || 'N/A'}</p>
                </div>
              </div>
              {data.accommodation.proofDocument && (
                <div>
                  <h5 className="text-xs font-medium mb-1 text-gray-600">Accommodation Proof</h5>
                  <div className="p-3 bg-gray-50 rounded-md border border-gray-100 flex justify-between items-center">
                    <div>
                      <p className="text-sm font-medium capitalize">{data.accommodation.proofDocument.type.replace('_', ' ')}</p>
                      <p className="text-xs text-gray-500">{data.accommodation.proofDocument.fileName}</p>
                      <p className="text-xs text-gray-500" suppressHydrationWarning>
                        Uploaded on {data.accommodation.proofDocument.uploadedAt ? new Date(data.accommodation.proofDocument.uploadedAt).toLocaleDateString() : 'N/A'}
                      </p>
                    </div>
                    <div className="flex items-center space-x-3">
                      {data.accommodation.proofDocument.verificationStatus && (
                        <Badge variant="outline" className={`
                          ${data.accommodation.proofDocument.verificationStatus === 'verified' ? 'bg-green-50 text-green-700 border-green-100' : 
                            data.accommodation.proofDocument.verificationStatus === 'pending' ? 'bg-amber-50 text-amber-700 border-amber-100' : 
                       ...
                       <p className="text-xs text-gray-500" suppressHydrationWarning>
                        Verified on {data.accommodation.proofDocument.verifiedAt ? new Date(data.accommodation.proofDocument.verifiedAt).toLocaleDateString() : 'N/A'}
                      </p>
          ...
          <div className="p-4 bg-white rounded-md border border-gray-200">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-sm font-medium">{data.bankName || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Account Holder</p>
                <p className="text-sm font-medium">{data.accountHolderName || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Account Number</p>
                <p className="text-sm font-medium">{data.accountNumber ? `****${data.accountNumber.slice(-4)}` : 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Sort Code</p>
                <p className="text-sm font-medium">{data.sortCode || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Account Type</p>
                <p className="text-sm">{data.accountType || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500">Currency</p>
                <p className="text-sm font-medium">{data.currency || 'N/A'}</p>
              </div>
            </div>
          </div>
        </div>
            ...
            {data.documents?.map((doc: any, index: number) => (
              <div key={index} className="p-3 bg-white rounded-md border border-gray-200 flex justify-between items-center">
                <div>
                  ...
                  <p className="text-xs text-gray-500" suppressHydrationWarning>
                    Uploaded by {doc.uploadedBy} on {doc.uploadedAt ? new Date(doc.uploadedAt).toLocaleDateString() : 'N/A'}
                  </p>
                </div>
                <div className="flex items-center space-x-3">
                  {doc.verificationStatus && (
                    <Badge variant="outline" className={`
                      ${doc.verificationStatus === 'verified' ? 'bg-green-50 text-green-700 border-green-100' : 
                        doc.verificationStatus === 'pending' ? 'bg-amber-50 text-amber-700 border-amber-100' : 
                        'bg-red-50 text-red-700 border-red-100'}
                    `}>
          ...
          <div>
            ...
            <div className="p-3 bg-white rounded-md border border-gray-200">
              <div className="grid grid-cols-2 gap-2 mb-2">
                <div className="flex items-center">
                  <div className={`h-2 w-2 rounded-full mr-1.5 ${data.verificationChecks.fundsVerified ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                  <span className="text-sm">Funds Verified</span>
                </div>
                <div className="flex items-center">
                  <div className={`h-2 w-2 rounded-full mr-1.5 ${data.verificationChecks.sourceOfFundsVerified ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                  <span className="text-sm">Source of Funds Verified</span>
                </div>
              </div>
              <p className="text-xs text-gray-500">
