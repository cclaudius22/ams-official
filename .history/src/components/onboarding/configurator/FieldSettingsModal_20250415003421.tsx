        ...
        <div className="grid gap-6 py-4 flex-1 overflow-y-auto pr-3"> {/* Added scroll */}
           ...
           <div className="space-y-4">
              ...
              <div className="grid grid-cols-4 items-center gap-x-4 gap-y-3">
                <Label htmlFor={`field-settings-label-${fieldId}`} className="text-right col-span-1">Label*</Label>
                <Input id={`field-settings-label-${fieldId}`} value={localFieldData.label || ''} onChange={(e) => handleChange('label', e.target.value)} className="col-span-3 h-9" placeholder="User-facing label" />

                {isTextualType && (<>
                     <Label htmlFor={`field-settings-placeholder-${fieldId}`} className="text-right col-span-1">Placeholder</Label>
                     <Input id={`field-settings-placeholder-${fieldId}`} value={localFieldData.placeholder || ''} onChange={(e) => handleChange('placeholder', e.target.value)} className="col-span-3 h-9" placeholder="Optional placeholder text" />
                </>)}
                      ...
                      <div key={index} className="flex items-center space-x-2 group">
                         ...
                         <div className='flex-1 grid grid-cols-2 gap-2'>
                            <div className='space-y-0.5'>
                                <Label htmlFor={`option-label-${fieldId}-${index}`} className='text-xs font-normal'>Display Label</Label>
                                <Input
                                    id={`option-label-${fieldId}-${index}`}
                                    value={option.label}
                             ...
                             <div className='space-y-0.5'>
                                <Label htmlFor={`option-value-${fieldId}-${index}`} className='text-xs font-normal'>Stored Value*</Label>
                                <Input
                                    id={`option-value-${fieldId}-${index}`}
                                    value={option.value}
