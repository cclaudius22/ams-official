...
function ConfiguratorUI() {
  // *** useConfigurator HOOK USED HERE ***
  const { state, dispatch } = useConfigurator(); // Correct place to use the hook
    ...
    if (over && active.id !== over.id) {
      ...
      if (activeDataType === 'step' && overDataType === 'step') {
        ...
        if (activeStepId && activeStepId === overStepId && activeStepId === state.activeStepId) {
           ...
           console.warn("Attempted field drag operation across different steps or inactive step.");
           toast({ title: "Cannot Move Field", description: "Fields can only be reordered within the same active step.", variant: "destructive"})
        }
      }
       else {
         console.log("Drag operation ignored - items are not of compatible types or not moved.");
       }
    } else {
        console.log("Drag ended without moving over a valid target.");
    }
  };
