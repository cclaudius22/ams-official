...
import SortableFieldItem from './SortableFieldItem'; // Import the updated component
// Import DND components later
...
const StepEditorPanel = () => {
  ...
  // Update global state only on blur to avoid excessive dispatches
  ...
  return (
    <div className="h-full flex flex-col">
      <Card className="flex-1 flex flex-col shadow-sm"> {/* Add subtle shadow */}
        {/* Step Header */}
        <CardHeader className="pb-4 border-b">
          <div className="space-y-1">
             ...
             <Textarea
                ...
                onBlur={updateStepDetails}
                className="text-sm text-muted-foreground border-0 shadow-none p-0 h-auto resize-none focus-visible:ring-0 mt-1" // Added mt-1
            ...
            {/* DND Context for fields will go here */}
             {/* <SortableContext items={activeStep.fields.map(f => f.id)} strategy={verticalListSortingStrategy}> */}
                {activeStep.fields.length === 0 ? (
                    <div className="border border-dashed rounded-md p-8 text-center text-muted-foreground bg-white"> {/* Added bg */}
                    ...
                    activeStep.fields
                        ...
                        .map((field) => (
                          // Use the actual SortableFieldItem component now
                         <SortableFieldItem
                            key={field.id}
                            field={field}
                            stepId={activeStep.id} // Pass stepId
                         />
                     ))
                 )}
            {/* </SortableContext> */}
