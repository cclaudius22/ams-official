...
interface VisaBuilderStep3Props {
  ...
  onPreviousStep: () => void;
  onSave: () => void;
  aiScans?: { id: string; name: string; enabled: boolean; }[]; // Make it optional
}
...
const VisaBuilderStep3: React.FC<VisaBuilderStep3Props> = ({
  ...
  onPreviousStep,
  onSave,
  aiScans: propAiScans // Rename to avoid conflict
}) => {
  const { visaConfig } = useVisaBuilder();
  const [showDialog, setShowDialog] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  
  // Use AI Scans from props if provided, otherwise use default values
  const defaultAiScans = [
  ...
  // Use prop value if provided, otherwise use default
  const aiScans = propAiScans || defaultAiScans;

  // We no longer need toggleAIScan since we're using the one from parent
  // This is now handled in the parent component via the AIScanConfigurator
  ...
  return (
    <div className="space-y-6">
      ...
      {/* AI Scan Configuration Controls moved to the right column in VisaBuilder.tsx */}
