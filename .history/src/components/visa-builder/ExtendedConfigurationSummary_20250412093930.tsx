...
import { FileText, ListChecks, UserCheck, BadgeInfo, MapPin, DollarSign, Clock, Cog, AlertTriangle, ShieldCheck } from 'lucide-react';
...
const ExtendedConfigurationSummary: React.FC<ExtendedConfigurationSummaryProps> = ({
  ...
  currentStep = 3, // Default to step 3
}) => {
  ...
  return (
    <Card className="bg-white">
      <CardHeader>
        ...
        <p className="text-xs text-gray-500">Final review of the visa type configuration.</p>
      </CardHeader>
      <CardContent className="space-y-5 text-sm">
        ...
        <div className="p-3 rounded-md border border-gray-100">
          ...
          {eligibilityCriteria.filter(c => c.trim() !== '').length > 0 ? (
            <ul className="list-disc space-y-1 pl-10 text-xs text-gray-600 border-l-2 border-teal-100 ml-2">
        ...
        {/* --- Step 3 Summary (AI Scans) --- */}
        <div className="mt-4 p-3 rounded-md border border-purple-100 bg-purple-50">
          <h3 className="font-semibold mb-2 flex items-center text-gray-700 text-base">
            <ShieldCheck className="h-4 w-4 mr-2 text-purple-600"/>
            AI Scan Requirements
          </h3>
          <p className="text-xs text-gray-600 mb-2 pl-6 ml-2">
            The following AI validations will be performed on submitted documents.
          </p>
          <div className="pl-6 border-l-2 border-purple-200 ml-2">
            <p className="text-sm text-purple-700">
              AI scan configuration is managed in the right panel.
            </p>
          </div>
        </div>
