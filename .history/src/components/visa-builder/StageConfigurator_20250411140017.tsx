...
const StageConfigurator: React.FC<StageConfiguratorProps> = ({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>2. Application Flow Builder</CardTitle>
        <p className="text-sm text-gray-500">Define the steps applicant follows.</p>
      </CardHeader>
      <CardContent>
        {/* --- Fixed Stages --- */}
        <div className="mb-6">
        ...
        <div className="mb-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 gap-2">
            <h3 className="font-medium flex items-center text-sm text-gray-700">
           ...
           <div className="space-y-2">
            {finalStages.map((stage) => {
              ...
              return (
                <div
                  key={stage.id}
                  className="border rounded-md p-3 bg-green-50 text-gray-700 flex items-center text-sm"
                >
                  <IconComponent className="h-5 w-5 text-green-600 mr-2 flex-shrink-0" />
                   <span>{stage.name}</span>
                   {/* Optional: Add a small indicator */}
                   {/* <span className="ml-auto text-xs text-green-500">(Final)</span> */}
                </div>
              );
            })}
          </div>
