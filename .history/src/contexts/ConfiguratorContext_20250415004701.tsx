...
export type ConfiguratorAction =
  ...
  | { type: 'ADD_FIELD'; payload: { stepId: string; fieldType: FieldType | 'heading' } }
...
function ensureOrderAndArrays(config: OnboardingConfiguration): OnboardingConfiguration {
...
function configuratorReducer(state: ConfiguratorState, action: ConfiguratorAction): ConfiguratorState {
  ...
  switch (action.type) {
    case 'VALIDATE_DETAILS':
      return { ...state, detailsValidated: true };
    ...
    case 'REORDER_STEPS': {
        ...
        // Ensure arrayMove is imported: import { arrayMove } from '@dnd-kit/sortable';
        const reorderedSteps = arrayMove(state.configuration.steps, oldIndex, newIndex)
            .map((step, index) => ({ ...step, order: index })); // Update order based on new position
